...
const ViewAllBookings = () => {
  ...
  const currentRecords = sortedBookings.slice(indexOfFirstRecord, indexOfLastRecord);
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {currentRecords.map((booking) => (
                    <tr key={booking._id}>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.userId?.name || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.eventId?.title || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.totalTicket}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.eventId?.location || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        {booking.eventId?.date ? new Date(booking.eventId.date).toLocaleDateString() : 'N/A'}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.createdAt || 'N/A'}</td>
                      <td className="px-6 py-4 whitespace-nowrap">{booking.status}</td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <Link to={`/update-booking/${booking._id}`} className="text-blue-500 hover:text-blue-700 mr-4">
                          Update
                        </Link>
                      </td>
                    </tr>
